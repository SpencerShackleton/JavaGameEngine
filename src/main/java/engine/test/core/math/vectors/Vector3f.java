...
public class Vector3f {
	...
	public Vector3f rotate(Quad rotation)
	{
		Quad conjugate = rotation.conjugate();

		Quad w = rotation.mult(this).mult(conjugate);

		return new Vector3f(w.getX(), w.getY(), w.getZ());
	}
	
	public Vector3f reflect(Vector3f normal) {
		return this.sub(normal.mult(this.dot(normal) * 2));
	}
