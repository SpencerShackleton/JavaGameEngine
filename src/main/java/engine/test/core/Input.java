...
public class Input {
	...
	{
		for(int i = 32; i < NUM_KEYCODES; i++)
			lastKeys[i] = getKey(i);
		
		for(int i = 32; i < NUM_MOUSEBUTTONS; i++)
	...
	{
	    DoubleBuffer posX = Util.createDoubleBuffer(1);
	    DoubleBuffer posY = Util.createDoubleBuffer(1);
	...
	{
		int c = enabled ? GLFW_CURSOR_NORMAL : GLFW_CURSOR_DISABLED;
		glfwSetInputMode(Window.getWindow(), GLFW_CURSOR, c);
	}
}
